...
<tr>
<td colspan="3">Business Name: <span id="application_taxpayer_business_name"></span></td>
</tr>
<tr>
...
<tr>
<td style="width: 50%;"><p style="padding-top: 5px;"><span id="printed_name" style="font-size: 16px;"></span></p><p style="margin-top: 5px;">SIGNATURE OF APPLICANT OVER PRINTED NAME</p></td>
<td style=""><div style="margin-top: 10px;"><p id="application_position_title" style="margin-bottom: 2px;"></p><p>POSITION/TITLE</p></div></td>
</tr>
</table>
...
$.ajax({
	...
	success: function(data, status) {		
		$.each(data, function(key, value) {
			if ($('#' + key)[0]) {
				$('#' + key).html(value);
				$('#' + key).css({"padding-left":"10px","font-size":"12px","font-weight":"bold"});				
			}
			...
			if (key == 'application_entity') {
				if (value) $('#tax_incentive_yes').html('*');
				else $('#tax_incentive_no').html('X');
			}
			if (key == 'application_business_rented') {
				if (value) $('#' + value).prop('checked',true);
			}
			if (key == 'application_monthly_rental_p') $('#application_monthly_rental').html(value);
		});
...
$.ajax({
	...
	success: function(data, status) {	
		$.each(data.aba_items, function(i,d){	
			...
			ba_row = '<tr>';
			ba_row += '<td style="text-align: center; font-size: 12px; font-weight: bold;">' + d.aba_code_name + '</td>';
			ba_row += '<td style="text-align: center; font-size: 12px; font-weight: bold;">' + d.aba_b_line_name + '</td>';
			ba_row += '<td style="text-align: center; font-size: 12px; font-weight: bold;">' + d.aba_units + '</td>';
			if (d.aba_gross_base == 'capitalization')
			ba_row += '<td style="text-align: center; font-size: 12px; font-weight: bold;">' + d.aba_gross_amount + '</td>';			
			else
			ba_row += '<td>&nbsp;</td>';
			...
			ba_row += '<td style="text-align: center;">' + d.aba_gross_amount + '</td>';
			else
			ba_row += '<td>&nbsp;</td>';
			ba_row += '</tr>';

			$('#business-activity tbody').html(ba_row);
